    ...
    def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray, batch) -> np.ndarray:
        ...
        for idx in batch:
            ...
            for n in range(self.n_class):
                if y_i != n and (np.dot(self.w[y_i], X_train[idx]) - np.dot(self.w[n], X_train[idx])) < 1:
                    w_b[y_i] += self.alpha*X_train[idx]
                    w_b[n] -= self.alpha*X_train[idx]
                w_b[n] -= self.alpha*(self.reg_const / X_train.shape[0])
        return w_b
